    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-lg border border-border/50 bg-muted/30 p-2.5">
          ...
          <Badge variant="outline" className={`text-[10px] mt-1 ${getSignalBg(indicators.rsi.signal)}`}>{indicators.rsi.signal}</Badge>
        </div>
        <div className="rounded-lg border border-border/50 bg-muted/30 p-2.5">
          ...
          <Badge variant="outline" className={`text-[10px] mt-1 ${getSignalBg(indicators.macd.signal)}`}>{indicators.macd.signal}</Badge>
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={`rounded-lg border p-3 ${getSignalBg(verdict.signal)}`}>
        <div className="flex items-center justify-between mb-1">
          <span className="text-sm font-bold flex items-center gap-1.5"><Zap className="w-3.5 h-3.5" />{verdict.signal}</span>
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-4 rounded-lg border border-border/50 bg-muted/30 p-3">
        <div className="relative w-14 h-14">
          <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
            ...
            <circle cx="18" cy="18" r="15.9" fill="none" strokeWidth="3" strokeDasharray={`${data.overallScore} ${100 - data.overallScore}`} strokeLinecap="round" className={data.overallScore >= 70 ? 'stroke-green-400' : data.overallScore >= 40 ? 'stroke-yellow-400' : 'stroke-red-400'} />
          </svg>
          <span className={`absolute inset-0 flex items-center justify-center text-sm font-bold ${getScoreColor(data.overallScore)}`}>{data.overallScore}</span>
        </div>
        <div className="flex-1">
          ...
          <Badge variant="outline" className={`text-[10px] mt-1 ${getSignalBg(data.assessment?.outlook || '')}`}>{data.assessment?.outlook || 'Neutral'}</Badge>
        </div>
      </div>
      ...
      {data.risks && data.risks.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[10px] text-muted-foreground font-medium flex items-center gap-1"><Shield className="w-3 h-3" /> Top Risks</p>
          {data.risks.slice(0, 2).map((r, i) => (
            <div key={i} className="flex items-center gap-2">
              <Badge variant="outline" className={`text-[9px] ${getSeverityColor(r.severity)}`}>{r.severity}</Badge>
...
function AssetBadge({ asset }: { asset: CryptoAsset }) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 p-2">
      ...
      <span className="text-xs text-muted-foreground">${asset.price.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
    </div>
  );
}

function SelectAssetPrompt({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-6 text-center">
      <BarChart3 className="w-8 h-8 text-muted-foreground/50 mb-2" />
      <p className="text-xs text-muted-foreground">Select an asset from the table below to run <span className="font-semibold text-foreground">{label}</span>.</p>
    </div>
  );
}
...
export function MarketInsightsPanel({ assets, selectedAssetId: externalSelectedId }: MarketInsightsPanelProps) {
  ...
  const selectedAssetId = externalSelectedId || '';
  const {
    technicalData,
    fundamentalData,
    isLoadingTechnical,
    ...
    <Card className="glass-card overflow-hidden">
      <Tabs defaultValue="sentiment" className="w-full">
        ...
        <TabsContent value="technical" className="mt-0 p-3 space-y-3">
          {!selectedAsset ? (
            <SelectAssetPrompt label="Technical Analysis" />
          ) : isLoadingTechnical ? (
            ...
            <>
              ...
              <Button className="w-full gap-2" size="sm" onClick={() => navigate(`/analysis/${selectedAssetId}`)}>
        ...
        <TabsContent value="fundamental" className="mt-0 p-3 space-y-3">
          {!selectedAsset ? (
            <SelectAssetPrompt label="Fundamental Analysis" />
          ) : isLoadingFundamental ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2">
              <Loader2 className="w-6 h-6 animate-spin text-primary" />
            ...
            <>
              <AssetBadge asset={selectedAsset} />
              <Button className="w-full gap-2" size="sm" onClick={handleRunFundamental}>
